    ...
    if len(tds) >= 3:
        ...
        <div class="sentence-card">
            ...
            <div class="sentence-actions">
                ...
                <button class="audio-btn" onclick="speakText('{english.replace('<span', '').replace('</span>', '').replace('class=', '').replace('"', '')}')">🔊 استماع</button>
            </div>
        </div>'''
...
with open('sentences_cards.html', 'w', encoding='utf-8') as f:
    f.write('\n'.join(cards_html))

print(f"✅ Created {len(cards_html)} sentence cards")
print("Saved to: sentences_cards.html")
